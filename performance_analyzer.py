...
def load_data_per_ticker(tickers):
    """Load the TEST split separately per ticker to inspect per-ticker behavior."""
...
def run_prediction(model, data_dict):
    """Run a forward pass to get logits and predictions."""
    model.eval()
...
def analyze_performance():
    ...
    macro_dim = dummy_train["s_m"].shape[-1] if dummy_train else 6
    ...
    model = StockMovementModel(
        ...
        device=DEVICE,
        dropout=0.0,
        class_weights=None,  # eval mode doesn't need weights -> avoids loss_fn.weight mismatch
    ...
    if len(unique_pred) == 1:
        ...
        print(f"   Model only predicts class {unique_pred[0]}")
