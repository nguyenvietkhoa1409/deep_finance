...
def train_model(train_data: dict, valid_data: dict, test_data: dict):
    ...
    for epoch in range(int(TrainConfig.epoch_num)):
        ...
        # Chỉ bắt đầu lưu best model sau warmup epochs
        if (epoch + 1) >= warmup_epochs:
            is_best = False
            if val_mcc > best_val_mcc:
                is_best = True
...
# --- 6. MAIN ---
if __name__ == "__main__":
    # SỬA ĐỔI 1: Trỏ đúng vào tên file đã merge đủ 4 mã
    pkl_path = os.path.join(GlobalConfig.PROCESSED_PATH, "unified_dataset_test.pkl")
    ...
    if use_kg:
        if use_hetero_kg:
            hybrid_kg_path = os.path.join(getattr(GlobalConfig, "KG_CACHE_DIR", "data/kg"), 'hetero_kg_graphs.pkl')
            kg_path = None
            print(f"🕸️  Using HYBRID Heterogeneous KG: {hybrid_kg_path}")
        else:
            kg_path = getattr(GlobalConfig, "KG_PROCESSED_PATH", "data/kg/processed")
            hybrid_kg_path = None
            print(f"📊 Using LEGACY KG embeddings: {kg_path}")
    else:
        kg_path = None
    ...
    dp = data_prepare(
        pkl_path, 
        # Nếu data_prepare gốc trong file của bạn không nhận kg_path/hybrid_kg_path thì xoá 3 dòng dưới
        # kg_data_path=kg_path, 
        # hybrid_kg_path=hybrid_kg_path,
        # use_hetero_kg=use_hetero_kg
    )
    
    # SỬA ĐỔI 2: Đọc linh hoạt toàn bộ các Ticker có trong GlobalConfig (4 mã)
    target_tickers = ['TSLA', 'AMZN', 'AAPL', 'MSFT']
    list_train, list_valid, list_test = [], [], []
    ...
    # Merge data của cả 4 mã lại với nhau
    final_train = merge_datasets(list_train, shuffle=True)
    final_valid = merge_datasets(list_valid, shuffle=False)
    final_test  = merge_datasets(list_test,  shuffle=False)

    if len(final_train) > 0:
        # SỬA ĐỔI 3: In ra kích thước data để kiểm tra (Sanity Check)
